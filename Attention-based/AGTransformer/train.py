...
# parser.add_argument('--dataset_name', default='debugdata', type=str)
...
parser.add_argument('--steps', default=[20], type=list)
...
def check_accuracy(
    ...
    for i in range(len(total_gt)):
        if total_pred[i] == total_gt[i]:
            right += 1
    accuracy = right / len(total_pred)
    metrics['loss']=sum(losses)/len(losses)
    # logger.info('batch size {}'.format(len(accuracy)))
    model.train()
    return metrics, accuracy, confumatrix
...
def main(args):
    ...
    for ifold, (train_index, valid_index) in enumerate(cv.split(data_dset)):
        ...
        scheduler=optim.lr_scheduler.MultiStepLR(optimizer, milestones=args.steps, gamma=0.1)
        ...
        if len(args.device_ids) > 1:
            if torch.cuda.device_count() >= len(args.device_ids):
                ...
                raise ValueError("the machine don't have {} gpus".format(str(len(args.device_ids))))
